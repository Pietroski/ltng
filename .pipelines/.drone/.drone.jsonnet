...
local unit_tests_cmd = [
	"GOMAXPROCS=1 go test -race $(go list ./... | grep -v /integration/ | grep -v /tests/ | grep -v /mocks/ | grep -v /schemas/ | grep -v /benchmark/)",
];
...
local integraiton_tests_cmd = [
	...
	"GOMAXPROCS=1 go test -race ./tests/integration/...",

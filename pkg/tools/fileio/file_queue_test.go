...
func BenchmarkFileQueueActionsConcurrent(b *testing.B) {
	...
	//	require.NoError(b, err)
	//	fq, err := New(ctx, ltngdata.GenericFileQueueFilePath, ltngdata.FQ)
	//	require.NoError(b, err)
	//
	//	op := syncx.NewThreadOperator("file_queue")
	//	limit := 5
...
func TestFileQueue_PopFromIndex(t *testing.T) {
	t.Run("single item", func(t *testing.T) {
		t.Run("index found", func(t *testing.T) {
			...
			path := filepath.Join(fileiomodels.GenericFileQueueFilePath, "pop_from_index")
			filename := fileiomodels.FQ

			err := os.RemoveAll(fileiomodels.GetFileQueueFilePath(fileiomodels.FileQueueFileVersion, path, filename))
			require.NoError(t, err)
	...
	t.Run("one after another", func(t *testing.T) {
		...
		t.Run("single-thread", func(t *testing.T) {
			t.Run("index found", func(t *testing.T) {
				...
				err := os.RemoveAll(fileiomodels.GetFileQueueFilePath(
					fileiomodels.FileQueueFileVersion, path, filename))
				require.NoError(t, err)

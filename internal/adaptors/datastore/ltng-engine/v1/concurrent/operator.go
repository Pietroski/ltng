...
func (e *LTNGEngine) createItem(
	...
	cidRespSignal := make(chan error, 1)
	iidForCid := iid.WithRespChan(cidRespSignal)
	ciidRespSignal := make(chan error, 1)
	iidForCiid := iid.WithRespChan(ciidRespSignal)
	ciildRespSignal := make(chan error, 1)
	iildForCiild := iid.WithRespChan(ciildRespSignal)

	e.mainThreadOperatorChan <- iidForCid
	e.indexThreadOperatorChan <- iidForCiid
	e.indexListThreadOperatorChan <- iildForCiild

	if err := ResponseAccumulator(cidRespSignal, ciidRespSignal, ciildRespSignal); err != nil {
		return nil, err
	}

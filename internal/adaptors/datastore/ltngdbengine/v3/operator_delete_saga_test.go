...
func initDeleteCascadeSagaTestSuite(t *testing.T) *testSuite {
	...
	ts.dci = newDeleteCascadeByIdxSaga(ts.ctx, ts.ds)
	ts.dio = newDeleteIdxOnlySaga(ts.ctx, ts.ds)
...
func TestDeleteSagaCascade_buildCreateItemInfoData(t *testing.T) {
	t.Run("buildDeleteItemInfoData - single item", func(t *testing.T) {
		...
		t.Run("should fail to delete cascade item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade indexed items with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade index list item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade relational data item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should delete cascade item without index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			...
			deleteInfoData := generateDeleteItemInfoDataFromItemInfoData(t, ts, itemInfoData, false)
			ops = ts.dcs.buildDeleteItemInfoData(ts.ctx, deleteInfoData)
			err = saga.NewListOperator(ops...).Operate()
			assert.NoError(t, err)
		...
		t.Run("should fail to delete cascade item without index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, false)
			ops := ts.us.buildUpsertItemInfoDataWithoutIndex(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade relational data item without index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, false)
			ops := ts.us.buildUpsertItemInfoDataWithoutIndex(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
...
func TestDeleteSagaCascadeByIndex_buildCreateItemInfoData(t *testing.T) {
	t.Run("buildDeleteItemInfoData - single item", func(t *testing.T) {
		t.Run("should delete cascade by index item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
			...
			deleteInfoData := generateDeleteItemInfoDataFromItemInfoDataCascadeByIndex(
				t, ts, itemInfoData, true)
			ops = ts.dcs.buildDeleteItemInfoData(ts.ctx, deleteInfoData)
			err = saga.NewListOperator(ops...).Operate()
			assert.NoError(t, err)
		...
		t.Run("should fail to delete cascade by index item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade by index indexed items with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade by index index list item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete cascade by index relational data item with index", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
...
func TestDeleteSagaIndexOnly_buildCreateItemInfoData(t *testing.T) {
	t.Run("buildDeleteItemInfoData - single item", func(t *testing.T) {
		t.Run("should delete index only", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
			...
			deleteInfoData := generateDeleteItemInfoDataFromItemInfoDataIndexOnly(
				t, ts, itemInfoData, true)
			ops = ts.dio.buildDeleteItemInfoData(ts.ctx, deleteInfoData)
			err = saga.NewListOperator(ops...).Operate()
			assert.NoError(t, err)
			...
			func() { // indexed items files
				indexingKeysMap := indexingKeysToMap(deleteInfoData.Opts.IndexingKeys)
				for _, indexKey := range itemInfoData.Opts.IndexingKeys {
					itemStrKey := hex.EncodeToString(indexKey)
					itemFilePath := ltngdbenginemodelsv3.GetDataFilepath(
						itemInfoData.DBMetaInfo.IndexInfo().Path, itemStrKey)

					if _, ok := indexingKeysMap[itemStrKey]; ok {
						file, err := osx.OpenFile(itemFilePath)
						assert.Error(t, err)
						assert.Nil(t, file)
			...
			func() { // index list file
				...
				// A - B
				newBytesList := bytesop.CalLeftDiff(
					itemInfoData.Opts.IndexingKeys, deleteInfoData.Opts.IndexingKeys)

				expectedFileData := ltngdbenginemodelsv3.NewFileData(
					itemInfoData.DBMetaInfo, &ltngdbenginemodelsv3.Item{
						Key: itemInfoData.Item.Key,
						Value: bytes.Join(newBytesList,
			...
			{ // indexed items memory
				...
				for _, indexKey := range itemInfoData.Opts.IndexingKeys {
					itemStrKey := hex.EncodeToString(indexKey)
					itemLockStrKey := itemInfoData.DBMetaInfo.IndexInfo().LockStrWithKey(itemStrKey)

					if _, ok := indexingKeysMap[itemStrKey]; ok {
						fileInfo, ok := ts.e.itemFileMapping.Get(itemLockStrKey)
						assert.False(t, ok)
						assert.Nil(t, fileInfo)
		...
		t.Run("should fail to delete index only from index items", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()
		...
		t.Run("should fail to delete index only from index list item", func(t *testing.T) {
			ts := initDeleteCascadeSagaTestSuite(t)
			itemInfoData := generateItemInfoData(t, ts, true)
			ops := ts.us.buildUpsertItemInfoData(ts.ctx, itemInfoData)
			err := saga.NewListOperator(ops...).Operate()

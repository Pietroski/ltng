...
# Run client tests
timeout 50s bash -c 'go clean -testcache && go test -v -run=TestClientsWithinDocker ./tests/benchmark/...' \
    >>"./docs/outputs/$TIMESTAMP.txt" || true
...
# Run engine benchmarks
timeout 50s bash -c 'go clean -testcache && go test -v -run=BenchmarkAllEngines -bench=BenchmarkAllEngines ./tests/benchmark/...' \
    >>"./docs/outputs/$TIMESTAMP.txt" || true
